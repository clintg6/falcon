...
class TestTorchProfiler(unittest.TestCase):
    ...
    def test_linear_module(self):
        # Test nn.Linear
        layer = nn.Linear(in_features=10, out_features=5).to(self.device)
        x = torch.randn(2, 10, dtype=torch.float16).to(self.device)
        ...
        self.assertEqual(logs[0]['input_dtype'], 'torch.float16')
        self.assertEqual(logs[0]['output_shape'], '(2, 5)')
        self.assertEqual(logs[0]['output_dtype'], 'torch.float16')
    ...
    def test_conv2d_module(self):
        # Test nn.Conv2d
        layer = nn.Conv2d(in_channels=3, out_channels=6, kernel_size=3).to(self.device)
        x = torch.randn(2, 3, 16, 16, dtype=torch.float16).to(self.device)
        ...
        # Check benchmarking
        time = self.profiler.benchmark_layer('Conv2d', (2, 3, 16, 16), 'float16', 
                                           {'in_channels': 3, 'out_channels': 6, 'kernel_size': 3})
        self.assertGreaterEqual(time, 0.0)

    def test_layernorm_module(self):
        # Test nn.LayerNorm
        layer = nn.LayerNorm(normalized_shape=10).to(self.device)
        x = torch.randn(2, 10, dtype=torch.float16).to(self.device)
        ...
        # Check benchmarking
        time = self.profiler.benchmark_layer('LayerNorm', (2, 10), 'float16', {'normalized_shape': 10})
        self.assertGreaterEqual(time, 0.0)
